...
import MarkdownToolbar from '../../components/story-editor/MarkdownToolbar';
import MarkdownPreview from '../../components/story-editor/MarkdownPreview';
import NodeGraph from '../../components/story-editor/NodeGraph';
import ConfirmDialog from '../../components/story-editor/ConfirmDialog';
import LineNumberedTextarea from '../../components/story-editor/LineNumberedTextarea';
...
const StoryCreatorStep2 = ({ nodes, setNodes }) => {
  ...
  const addChoice = () => {
    const updatedNodes = [...nodes];
    updatedNodes[selectedNodeIndex].choices.push({ text: '', nextNodeId: '' });
    setNodes(updatedNodes);
  };
      ...
      <div className="nodes-sidebar">
        <div className="sidebar-header">
          ...
          <div className="sidebar-header-actions">
            <button type="button" onClick={addNode} className="btn btn-success btn-small">+ Add</button>
          </div>
        </div>
        ...
        <button type="button" onClick={() => setShowGraph(!showGraph)} className="map-toggle" title={showGraph ? 'Hide map' : 'Show map'}>
        ...
        <div className="nodes-list">
          {nodes
            ...
            .filter(({ node }) => node.name.toLowerCase().includes(sceneSearchQuery.toLowerCase()))
            .map(({ node, idx }) => (
              <div key={node.nodeId} className="node-item-container">
                <button className={`node-item ${idx === selectedNodeIndex ? 'active' : ''}`} onClick={() => setSelectedNodeIndex(idx)}>
                  <span className="node-badge">{idx === 0 ? '▶' : node.isEnding ? '■' : '○'}</span>
                  <span className="node-name">{node.name || node.nodeId}</span>
                </button>
                {idx > 0 && (
                  <button className="node-delete" onClick={() => removeNode(idx)} title="Delete this scene (requires confirmation)">
                    ✕
                  </button>
                )}
              </div>
          ...
          <div className="node-header">
            ...
            <div className="node-type-selector">
              {selectedNodeIndex === 0 ? (
                ...
                <button type="button" onClick={toggleNodeEnding} className="node-type-badge ending">Ending ✓</button>
              ) : (
                <button type="button" onClick={toggleNodeEnding} className="node-type-badge choice">Mark as Ending</button>
              )}
            </div>
          </div>
          ...
          <div className="content-editor">
            <div className="editor-label">
              ...
              <div className="editor-mode-buttons">
                <button type="button" onClick={() => setShowPreview(false)} className={`mode-btn ${!showPreview ? 'active' : ''}`} title="Edit markdown">
                  Edit
                </button>
                <button type="button" onClick={() => setShowPreview(true)} className={`mode-btn ${showPreview ? 'active' : ''}`} title="Preview formatted content">
                  Preview
                </button>
              </div>
            </div>

            {!showPreview && <MarkdownToolbar onInsert={handleMarkdownInsert} textareaRef={textareaRef} />}
            ...
            <div className={`editor-wrapper ${!showPreview ? 'with-lines' : ''}`}>
              {!showPreview ? (
                ...
                <MarkdownPreview content={currentNode.content} isVisible={true} />
              )}
            </div>
          </div>
                  ...
                  {currentNode.choices.map((choice, idx) => (
                    <div key={idx} className="choice-form">
                      <div className="choice-header">
                        ...
                        <button type="button" onClick={() => removeChoice(idx)} className="btn btn-danger btn-small">Remove</button>
                      </div>
